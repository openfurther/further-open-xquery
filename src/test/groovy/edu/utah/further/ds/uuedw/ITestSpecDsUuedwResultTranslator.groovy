 ...
 */
package edu.utah.further.ds.uuedw
...
@Slf4j
class ITestSpecDsUuedwResultTranslator extends TranslationTest {
	@Autowired
	...
	@Unroll
	def "Testing UUEDW Result Translations: #name"() {
		given:
		def xQuery = url.toURL().text

		def parameters = ["srcNmspcId" : "32776", "dataSetId" : "1234"]
                log.debug("++++++++++++++++++++++++++++++++++++++++")
                log.debug(url);
                log.debug(xQuery);
                log.debug("++++++++++++++++++++++++++++++++++++++++")
                ...
                try{
                        output = xQueryService.executeIntoString(
		...
		where:
		result << queryFiles('/result-translator/uuedw/input/*').values()
		expected << queryFiles('/result-translator/uuedw/expected/*').values();
		name << queryFiles('/result-translator/uuedw/input/*').keySet()
	}
}

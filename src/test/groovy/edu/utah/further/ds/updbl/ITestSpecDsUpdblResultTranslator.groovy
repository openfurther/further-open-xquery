 ...
 */
package edu.utah.further.ds.updbl
...
@Slf4j
class ITestSpecDsUpdblResultTranslator extends TranslationTest {
	@Autowired
	...
	def "Testing UUEDW Result Translations: #name"() {
		...
		def parameters = ["srcNmspcId" : "32774", "dataSetId" : "1234"]
                log.debug("++++++++++++++++++++++++++++++++++++++++")
                log.debug(url);
                log.debug(xQuery);
                log.debug("++++++++++++++++++++++++++++++++++++++++")
		...
		where:
		result << queryFiles('/result-translator/updbl/input/*').values()
		expected << queryFiles('/result-translator/updbl/expected/*').values();
		name << queryFiles('/result-translator/updbl/input/*').keySet()
	}
}
